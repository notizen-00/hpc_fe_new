...
export const useLabStores = defineStore('labStore', {
  state: () => ({
  ...
  actions: {
    ...
    async fetchPath()
    {
      ...
      try {
        ...
        const response = await axios.get(this.apiUrl + `user/user${authStore.user.uid}/api/contents`, {
          headers: {
            Authorization: `Bearer 580baeb7d5524bfbaa45f665e076a959`,
          },
        }); 
        console.log(response)
        if (response.status === 200) {
